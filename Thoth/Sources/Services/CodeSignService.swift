...
/// 2. ログインキーチェーンに証明書（"Thoth Local Signing"）があれば再利用し、
...
final class CodeSignService {
    ...
    /// Clipy 時代の証明書（"Clipy Local Signing"）とは独立した identity を使い、
    /// 旧 Clipy と Thoth を同時に運用できるようにする。
    /// 旧証明書で ACL が張られた Keychain エントリの移行読み出しでは
    /// macOS の許可ダイアログが表示されるので「常に許可」を選択すること
    static let certificateCommonName = "Thoth Local Signing"
    /// 再署名後の再起動時に付与する引数（再署名の無限ループ防止）
    static let resignedArgument = "--thoth-resigned"
    /// 再署名を無効化したい場合の引数（開発・デバッグ用）
    static let skipResignArgument = "--thoth-skip-resign"
    ...
    /// 現在のバイナリが安定署名（Thoth Local Signing）で署名されているか。
    ...
    private func createSigningIdentity() -> Bool {
        ...
        let workDirectory = fileManager.temporaryDirectory
            .appendingPathComponent("thoth-codesign-\(UUID().uuidString)", isDirectory: true)
        do {
        ...
        let passphrase = "thoth-local-signing"
