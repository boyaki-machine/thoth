...
- メニュー内では矢印キーおよび vim 風の `hjkl` キーで移動できます
- `/` キーで検索ボックスに移り、インクリメンタルサーチでアイテムを絞り込めます。検索対象はタイトル・ラベルに加えて、**マスクを掛けていないテキスト・URL の値とメモ本文**です（マスクを掛けた値と TOTP secret は対象外）。セキュア情報確認ウィンドウの検索も同じ条件です
...
| **直接貼り付け** | 選択した値を、ホットキーを押したときに使っていたアプリへそのまま貼り付け。Thoth のコピー履歴には残らず、貼り付けが済むとクリップボードは元の内容に戻ります。TOTP はキー入力を直接送出するため、どの履歴にも残りません |
...
| **メモ** | 契約番号・問い合わせ先・用途などの複数行メモ | **表示されません**（長文のため確認ウィンドウ専用）。ただし本文は検索の対象です |
| **TOTP** | ワンタイムパスワード | 現在のコードと残り秒数を表示・入力可 |

**ステップ1 — 認証情報を登録する**

メインメニュー（**⌘⇧V**）→「**セキュア情報**」（メニュー表示中に `s` キーでも起動）を選ぶと、Touch ID / パスワード認証のあと**セキュア情報ウィンドウ**が開きます。セキュアアイテム選択パネル（**⌘⇧.**）の一番下にある「**セキュア情報確認 (&s)**」からも同じウィンドウを開けます。
...
5. パスワードは「**パスワード生成...**」（**⌘G**）で作れます。生成ウィンドウの「**コピー**」でコピーし、値の欄に貼り付けてください（マスク中の欄は 👁 で平文にしてから貼り付けます）
6. 編集内容は自動で保存されます（**⌘S** で明示的に保存することもできます）

Export / Import は左ペイン下部の **⚙** メニューから行います。同じメニューに取り消し / やり直しもあります。
...
- 通常項目の貼り付けにはクリップボードを一時利用しますが、履歴に残らないマーカー（`org.nspasteboard.ConcealedType`）を付与し、**貼り付けの約 2 秒後に元のクリップボードの内容へ戻します**（貼り付けた値を ⌘V でもう一度貼り付けることはできません。その間に別の内容をコピーした場合は、そちらを残します）
- 環境設定の「メニュー項目選択後に "⌘ + V" を入力」がオフ、またはアクセシビリティ権限が無く自動で貼り付けられないときは、自分で貼り付けられるよう値をクリップボードに残し、30 秒後にクリアします
...
セキュア情報ウィンドウ（「**パスワード生成...**」/ **⌘G**）や指紋パスワード管理画面からも、パスワード生成を呼び出せます。どの画面から開いても内容は同じで、生成結果は「コピー」で受け取ります。
...
| **デスクトップフォルダ** | **任意** | ベータ機能の「スクリーンショットを履歴に保存」を有効にした場合のみ要求。スクリーンショットの保存先（既定はデスクトップ）に追加されたファイルを読むために必要になる。保存先をデスクトップ・書類・ダウンロード以外（例: `~/Pictures`）にしている場合は要求されない |

> デスクトップフォルダへのアクセスは**「スクリーンショットを履歴に保存」機能を有効にした場合にのみ**求められます（既定では無効）。この機能を使わない場合は拒否して構いません。クリップボード履歴・スニペット・セキュアアイテム・暗号化・パスワード生成など他の機能はすべて影響なく動作します。

> **「スクリーンショットを履歴に保存」について:** スクリーンショットの保存先フォルダ（⌘⇧5 の「オプション」→「保存先」で変えた場所にも追従します）を直接見張り、保存されたらすぐ履歴に追加します（v1.6.1 から。以前は Spotlight の索引を待つため、数秒〜数十秒遅れたり、取りこぼしたりしていました）。
> ただし macOS の「**フローティングサムネールを表示**」がオン（既定）の間は、画面右下のサムネールが消えるまで macOS がファイルを書き出さないため、履歴への追加も約 5 秒遅れます。すぐに追加したい場合は ⌘⇧5 →「オプション」でこの項目をオフにしてください。
> 監視を有効にする前に撮ったスクリーンショットや、保存先へ後から移したファイルは追加しません。
...
- **デバッグ情報を保存する（ベータ機能）**: 不具合の調査に協力してもらうときだけオンにする設定です（既定はオフ）。オンの間だけ、貼り付けとセキュアメニューの操作の段階（「認証に成功した」「貼り付け先が前面に来た」など）と時刻を `~/Library/Logs/Thoth/debug.log` に保存します。**コピーした内容・セキュアアイテムの値や項目名・使ったアプリの名前は保存しません。** 「表示」で中身をいつでも確認でき、オフにするとファイルを削除します。オフの間は何も記録しません。
- **セキュアアイテムの Export / Import**: セキュア情報ウィンドウの **⚙** メニューから、セキュアアイテムと指紋パスワードを JSON ファイルとして書き出し / 読み込みできます。複数端末で同じ情報を利用する用途に使えます（**書き出しファイルは平文のため取り扱いに注意してください**）。書き出されるファイルのスキーマは [docs/DESIGN_JP.md](docs/DESIGN_JP.md) を参照してください。
