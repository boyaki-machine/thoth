...
- 本アプリは Apple の公証（notarization）を受けていないため、他の Mac にコピーして初回起動すると Gatekeeper に「開発元を確認できません」等でブロックされます。最近の macOS では、**システム設定 → プライバシーとセキュリティ** を開き、下部の「"Thoth" は…ブロックされました」の横の **「このまま開く」** から起動を許可してください（初回のみ）。ターミナルで `xattr -dr com.apple.quarantine /Applications/Thoth.app` を実行しても回避できます。
