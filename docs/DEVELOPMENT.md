...
- **Code generation**: `L10n.*` (localized strings) and `Asset.*` (images) are generated by SwiftGen at build time. To add strings/assets, edit the source files (e.g. `Clipy/Resources/*.lproj/Localizable.strings`); `Clipy/Generated/` is regenerated at build time (no manual editing of generated files needed).
...
│   │   ├── Extensions/              Swift/Cocoa extensions (NSAlert+Clipy, etc.)
...
├── ClipyTests/                      Unit tests (Quick + Nimble)
...
### Services Layer (`Clipy/Sources/Services/`)
...
Auxiliary persistence utilities (`Clipy/Sources/Utility/`):
...
Unit tests (`ClipyTests` target / Quick + Nimble) are run via the scheme's Test action (Debug configuration).
...
- Tests use an in-memory Realm DB, UserDefaults, and the Keychain (a test-only service `com.clipy-app.ClipyTests.SecureMenu` that never touches production data), so no extra setup is required.
- `ClipyTests` uses `@testable import Clipy`, so `ENABLE_TESTABILITY=YES` is required when running outside the Test action (Debug configuration).
...
To run a single spec, use e.g. `-only-testing:ClipyTests/CryptoServiceSpec`.
...
Run SwiftLint (config: `.swiftlint.yml`, targets: `Clipy/Sources` and `ClipyTests`).
...
# → produced at build/DerivedData/Build/Products/Release/Clipy.app
```
...
To avoid this, the app checks its own signature at launch and automatically re-signs itself with a device-specific certificate named **Clipy Local Signing**.
...
- To disable re-signing, launch with the `--clipy-skip-resign` argument.
...
macOS Accessibility (TCC) identifies an app by "bundle ID + signing requirement", so repeatedly building ad-hoc-signed adds duplicate Clipy entries in System Settings.

This app performs the accessibility check **only after** re-signing completes (the pre-re-sign ad-hoc binary is never registered with TCC), so once you grant permission while signed with "Clipy Local Signing", the same entry is reused across later versions.

Old entries accumulated from past builds cannot be removed automatically, so reset them once with the following, then re-grant permission to the newly launched Clipy.
