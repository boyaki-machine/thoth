...
/// 実サービス（テスト用キーチェーン）を通して、構成変更と編集の保存フローを端から端まで確かめる
extension SecureInfoCommitFlowSpec {

    // MARK: - Structural changes (end to end)

    /// 指定のアイテムを保存した状態のウィンドウを組み立てる（構成変更の保存フロー用）
    static func makeStructuralSplitViewController(service: SecureMenuService,
                                                  with items: [SecureMenuItem]) -> CPYSecureInfoSplitViewController {
        for item in items {
    ...
    /// 1 アイテムを保存した状態のウィンドウを組み立て、そのアイテムの編集を始める（編集からの保存フロー用）
    static func makeCommitFlowSplitViewController(service: SecureMenuService) -> CPYSecureInfoSplitViewController {
    ...
    static func structuralFieldFlowSpecs() {
        describe("構成変更の保存フロー（フィールド）") {
            ...
            it("追加したフィールドが保存され、種別とマスク指定が保たれる") {
                let splitViewController = makeStructuralSplitViewController(service: service, with: [
    ...
    static func structuralItemFlowSpecs() {
        describe("構成変更の保存フロー（アイテム）") {
    ...
    static func structuralWindowFlowSpecs() {
        describe("構成変更の保存フロー（ウィンドウ・保存できない状態）") {
    ...
    static func commitFlowSaveSpecs() {
        describe("編集からの保存フロー（保存される）") {
            ...
            it("タイトルの編集がキーチェーンへ保存される") {
                let splitViewController = makeCommitFlowSplitViewController(service: service)
    ...
    static func commitFlowGuardSpecs() {
        describe("編集からの保存フロー（保存しない・保護）") {
