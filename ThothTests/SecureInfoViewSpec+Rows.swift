...
/// フィールド行の表示・URL を開く・詳細ペインの行・画面の組み立て
extension SecureInfoViewSpec {

    // MARK: - Field row rendering

    /// 行ビューが「値をどう見せるか」。TOTP secret とマスク値を
    /// 画面へ出さないことを固定する
    static func fieldRowSpecs() {
        describe("フィールド行の表示") {
    ...
    static func detailRowBuildingSpecs() {
        describe("詳細ペインの行構築") {
    ...
    static func viewIntegrationSpecs() {
        describe("ビューの組み立て") {
